 ...
 * @author Amal Mabrouk
 */
public class ListeCafeRestoVendeur extends Layout {

    EncodedImage enc;
    URLImage uRLImage;

    public ListeCafeRestoVendeur() {
        ...
        f.getToolbar().addCommandToOverflowMenu("Ajouter", null,
                (ev) -> {
                    AjouterCafe cf = new AjouterCafe();
                    cf.getF().show();
                });
        
        f.getToolbar().addCommandToOverflowMenu("Liste des CafeResto en attente", null,
                (ev) -> {
                    ListeCafeRestoVATT cf = new ListeCafeRestoVATT();
                    cf.getF().show();
                });
        ...
        ArrayList<CafeResto> listcafe = sc.getListCafeRestoV(MyApplication.user.getId());
    ...
    public Container addItem(CafeResto c) {
        ...
        libelle.addPointerPressedListener((act) -> {
            DetailsCafe int2 = new DetailsCafe(c);
            int2.getF().show();
        });
        Label ll = new Label("");
          Label lll = new Label("");
