 ...
 * @author Amal Mabrouk
 */
public class ListeCafeRestoVATT  extends Layout {

    EncodedImage enc;
    URLImage uRLImage;

    public ListeCafeRestoVATT() {
        ...
        ArrayList<CafeResto> listcafe = sc.getListCafeRestoVAtt(MyApplication.user.getId());
    ...
    public Container addItem(CafeResto c) {
        ...
        libelle.addPointerPressedListener((act) -> {
            DetailscafeATT int2 = new DetailscafeATT(c);
            int2.getF().show();
        });
        Label ll = new Label("");
          Label lll = new Label("");
